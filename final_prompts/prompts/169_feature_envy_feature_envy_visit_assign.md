...
- Function 'visit_Assign' mutates multiple variables in other modules: self.result.data_flows[scoped_name].dependencies, self.result.mutations, self.variable_uses[scoped_dep].
- Status reachability: unknown
- Feature Envy: Metoda używa więcej danych z `self` niż z `dfg`.
- Obce Mutacje: self.result.data_flows[scoped_name].dependencies, self.result.mutations, self.variable_uses[scoped_dep]
...
## Instrukcja
Przenieś metodę visit_Assign do modułu, który posiada większość używanych w niej danych. Zmniejsz coupling między modułami.