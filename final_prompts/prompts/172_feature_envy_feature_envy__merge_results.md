## Cel refaktoryzacji
Move method `_merge_results` from module `analyzer` to `merged`.

## Powód (Głęboka Analiza)
- Function '_merge_results' mutates multiple variables in other modules: merged.functions, merged.classes, merged.nodes, merged.mutations, merged.edges, merged.data_flows.
- Status reachability: unknown
- Feature Envy: Metoda używa więcej danych z `merged` niż z `analyzer`.
- Obce Mutacje: merged.functions, merged.classes, merged.nodes, merged.mutations, merged.edges, merged.data_flows

## Kontekst strukturalny
- Zależności: 
- Mutacje w module docelowym: This code mutates state in merged
...
## Instrukcja
Przenieś metodę _merge_results do modułu, który posiada większość używanych w niej danych. Zmniejsz coupling między modułami.